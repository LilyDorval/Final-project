...
#run SDT model and get posterior samples
data = read_data(file_to_print, display= True)
with apply_hierarchical_sdt_model(data) as model:
    trace = pm.sample(draws=1000, tune=1000, target_accept=0.9, return_inferencedata=True)

#check model convergence
az.summary(trace, var_names=['mean_d_prime', 'mean_criterion'])
az.plot_trace(trace, var_names=['mean_d_prime', 'mean_criterion']);

#compare posterior distribution
dprime = trace.posterior['mean_d_prime']
criterion = trace.posterior['mean_criterion']

#Effect of Stimulus Type
delta_stim_easy = dprime.sel(mean_d_prime_dim_0=1) - dprime.sel(mean_d_prime_dim_0=0)
delta_stim_hard = dprime.sel(mean_d_prime_dim_0=3) - dprime.sel(mean_d_prime_dim_0=2)
#Effect of Trial Difficulty
delta_diff_simple = dprime.sel(mean_d_prime_dim_0=2) - dprime.sel(mean_d_prime_dim_0=0)
delta_diff_complex = dprime.sel(mean_d_prime_dim_0=3) - dprime.sel(mean_d_prime_dim_0=1)

#Visualization
# Create a 2x2 subplot layout
fig, axes = plt.subplots(2, 2, figsize=(12, 10))

# Plot 1: Stimulus effect in easy trials
az.plot_posterior(delta_stim_easy, hdi_prob=0.95, ax=axes[0, 0])
axes[0, 0].set_title("Stimulus Type Effect (Easy Trials)")

# Plot 2: Stimulus effect in hard trials
az.plot_posterior(delta_stim_hard, hdi_prob=0.95, ax=axes[0, 1])
axes[0, 1].set_title("Stimulus Type Effect (Hard Trials)")

# Plot 3: Difficulty effect in simple stimuli
az.plot_posterior(delta_diff_simple, hdi_prob=0.95, ax=axes[1, 0])
axes[1, 0].set_title("Difficulty Effect (Simple Stimuli)")

# Plot 4: Difficulty effect in complex stimuli
az.plot_posterior(delta_diff_complex, hdi_prob=0.95, ax=axes[1, 1])
axes[1, 1].set_title("Difficulty Effect (Complex Stimuli)")

# Tight layout to prevent overlap
plt.tight_layout()
plt.show()

# Summarize the group-level SDT parameters
summary_df = az.summary(trace, var_names=["mean_d_prime", "mean_criterion"], hdi_prob=0.95)

# Display or export the table
print(summary_df)
summary_df.to_csv("sdt_posterior_summary.csv")  # Optional: Save to CSV